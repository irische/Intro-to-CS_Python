...
def in_common( state1, state2 ):
    ...
    Starting by making a list from the intersect of the two sets of
    ...
    if len(common) == 0:
        print "States %s and %s have no companies in common" %(state1[1], state2[1])
    else:
        print "States %s and %s have %d companies in common" %(state1[1], state2[1], len(common))
        print "Here they are in alphabetical order:"
        i = 0
        for i in range(len(common)):
            print '\t%d: %s' %(i,common[i])
...
def only_in_state( state, all_states):
    ...
    if len(only_in_state) == 0:
        print 'No companies are only in state %s' %state[0]
    else:
        #  Output on one line.
        out_string = ''
        for s_name in only_in_state:
            out_string += s_name + ' '
        out_string.strip()  # get rid of last blank
        print "The following companies are only in state %s: %s" %(state[0], out_string)


if __name__ == "__main__":
    '''
    Build the list of lists of states
    '''
    all_states = []
    ...
    companies = set(['ibm', 'google', 'bloomberg', 'goldman'])
    ...
    print "The least populous state in the list, with rank %d, is %s"\
        %(all_states[0][0], all_states[0][1])
